...
pub async fn upload_session_chunk_handler(
    ...
    let bytes = to_bytes(request.into_body(), state.config.max_upload_bytes)
        .await
        .map_err(|_| ApiError::bad_request("Invalid chunk payload."))?;
    Ok(json_response(
        state
            .upload
            .append_chunk(&session_id, bytes.to_vec())
            .await?,
    ))
}
...
pub async fn media_tracks_handler(
    ...
    if let Ok(probe) = state.media.probe_media_tracks(&source_input).await {
        if let Some(audio_track) = choose_audio_track_from_probe(&probe, &preferred_audio_lang) {
...
mod tests {
    use super::{
        build_playback_session_key, find_episode_pattern, normalize_preferred_audio_lang,
