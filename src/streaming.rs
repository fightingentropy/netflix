...
impl StreamingService {
    ...
    pub async fn create_remux_response(
        ...
        if safe_subtitle_stream_index >= 0 {
            ffmpeg_args.push("-map".to_owned());
            ffmpeg_args.push(format!("0:{safe_subtitle_stream_index}?"));
        } else {
        ...
        if safe_subtitle_stream_index >= 0 {
            ...
            ffmpeg_args.push("-disposition:s:0".to_owned());
            ffmpeg_args.push("default".to_owned());
        }
        ffmpeg_args.extend([
...
#[cfg(test)]
mod tests {
    use super::{build_hls_transcode_job_key, normalize_remux_video_mode};
