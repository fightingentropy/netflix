...
const OPENSUBTITLES_DEFAULT_USER_AGENT: &str = "TemporaryUserAgent";
...
fn is_allowed_external_subtitle_download_url(download_url: &str) -> bool {
    Url::parse(download_url)
        ...
        .and_then(|url| {
            ...
            Some(hostname == "dl.opensubtitles.org" || hostname.ends_with(".opensubtitles.org"))
        })
        .unwrap_or(false)
}
...
fn sort_subtitle_tracks_by_playback_preference(tracks: Vec<SubtitleTrack>) -> Vec<SubtitleTrack> {
    ...
    tracks.sort_by(|left, right| {
        ...
        left_forced
            ...
            .then_with(|| left.streamIndex.cmp(&right.streamIndex))
    });
    tracks
}
