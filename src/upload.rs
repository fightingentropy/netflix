...
impl UploadService {
    ...
    pub async fn append_chunk(&self, session_id: &str, chunk: Vec<u8>) -> AppResult<Value> {
        self.sweep_sessions().await;
        ...
        self.ensure_upload_directories().await?;
        append_file(&temp_path, &chunk).await?;
...
async fn append_file(path: &Path, bytes: &[u8]) -> AppResult<()> {
...
mod tests {
    use super::{
        ...
        is_allowed_gallery_download_url, normalize_gallery_download_url,
    };
