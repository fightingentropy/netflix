...
pub async fn read_local_library(path: &Path) -> Library {
...
async fn read_local_library_unlocked(path: &Path) -> Library {
    match fs::read_to_string(path).await {
        Ok(raw) => {
            let parsed = serde_json::from_str::<Value>(&raw).unwrap_or(Value::Null);
            normalize_local_library(parsed)
        }
...
mod tests {
    ...
    async fn mutates_local_library_without_losing_concurrent_entries() {
        ...
        let library = read_local_library(&path).await;
